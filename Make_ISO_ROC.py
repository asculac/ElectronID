...
#      for location in ['EB1', 'EB2', 'EE']:
         ...
         q = np.linspace(0.2, 0.65, 50)[1:-1]
         id_quantiles = df.Spring16HZZV1RawVals.quantile(q)

         i = 0
         for cut in id_quantiles:
            df_cut = df_eta_pt[df_eta_pt.Spring16HZZV1RawVals > cut]
...
#            print sig_eff
#            print bkg_eff
#            print df_cut["hzz_iso"]
            
            fpr, tpr, _ = metrics.roc_curve(df_cut["y"] == 1, df_cut["hzz_iso"], pos_label=1)
            tpr = sig_eff * tpr * 100
            fpr = bkg_eff * fpr * 100
...
#         plt.show()
         plt.savefig(join(plot_dir, "ISO_after_ID_ROC/2016_{0}_{1}.pdf".format(location, ptrange)), bbox_inches='tight')
         plt.savefig(join(plot_dir, "ISO_after_ID_ROC/2016_{0}_{1}.eps".format(location, ptrange)), bbox_inches='tight')
         os.system("convert -density 150 -quality 100 " + join(plot_dir, "ISO_after_ID_ROC/2016_{0}_{1}.eps".format(location, ptrange)) + " "
                                                        + join(plot_dir, "ISO_after_ID_ROC/2016_{0}_{1}.png".format(location, ptrange)))         
