...
if ROC:
   ...
   for ptrange in ["10"]: #for ptrange in ["5", "10"]:
      
      for location in ["EE"]: #for location in ["EB1", "EB2", "EE"]:

         print("processing {0} {1}...".format(location, ptrange))

         h5_file = h5_dir + "/Autumn_18_ID_ISO/" + location + "_" + ptrange + '/pt_eta_score.h5'
         h5_file_ul = h5_dir + "/Summer_18UL_ID_ISO/" + location + "_" + ptrange + '/pt_eta_score.h5'
            
         df = load_df(h5_file)
         df_ul = load_df(h5_file_ul)
         ...
         for yr, lbl in roc_curves:
            ...
            if lbl == "UltraLegacy":
                y, x, _ = metrics.roc_curve(df_ul["y"] == 1, df_ul["bdt_score_default"])
            x = x * 100
            y = y * 100
